...
import { runVulnerabilityScanner, runLinter, runASTAnalyzer, } from "./scanners/index.js";
...
program
    ...
    if (isText)
        console.log(chalk.gray(`Loaded AgentLint config (Found ${Object.keys(config.rules).length} rule overrides)\n`));
    // Gather Data
...
function printReport(vuln, lint, ast) {
    ...
    if (lint.errorCount > 0 || lint.warningCount > 0) {
        const errorStr = lint.errorCount > 0
            ? chalk.red(`${lint.errorCount} Errors`)
            : `0 Errors`;
        const warnStr = lint.warningCount > 0
        ...
        lint.messages.forEach((result) => {
            if (result.messages.length > 0) {
                ...
                result.messages.forEach((msg) => {
                    ...
                    console.log(`    ${icon} ${color(`[Line ${msg.line}]`)} ${msg.message} ${chalk.gray(`(${msg.ruleId})`)}`);
                    if (msg.fix) {
                        console.log(`      💡 ${chalk.italic("Suggestion:")} Auto-fix available via 'agentlint fix'.`);
                    }
                    else {
                        console.log(`      💡 ${chalk.italic("Suggestion:")} Review ESLint rule '${msg.ruleId}' to resolve this issue.`);
                    }
                });
                console.log();
            }
    ...
    for (const cat of categories) {
        ...
        if (catIssues.length > 0) {
            ...
            console.log(subDivider);
            const grouped = catIssues.reduce((acc, issue) => {
                if (!acc[issue.file])
                    acc[issue.file] = [];
            ...
            for (const [file, issues] of Object.entries(grouped)) {
                ...
                issues.forEach((issue) => {
                    ...
                    console.log(`    ${icon} ${color(`[Line ${issue.line}]`)} ${issue.message} ${chalk.gray(`(${issue.ruleId})`)}`);
                    if (issue.suggestion) {
                        console.log(`      💡 ${chalk.italic("Suggestion:")} ${issue.suggestion}`);
                    }
                });
    ...
    console.log(divider);
    const summaryColor = vuln.issues > 0 ||
        lint.errorCount > 0 ||
        ast.some((i) => i.severity === "error")
        ? chalk.red
        : chalk.green;
    console.log(summaryColor(`✅ Scan complete. Found ${vuln.issues} vulnerabilities, ${lint.errorCount} lint errors, ${lint.warningCount} lint warnings, and ${ast.length} agentic smells.\n`));
}
program
    .command("fix")
