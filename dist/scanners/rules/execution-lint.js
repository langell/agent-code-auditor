export function checkExecutionRules(file, lines, config) {
    const issues = [];
    ...
    if (hasWhileTrue && !hasMaxSteps) {
        issues.push({
            file,
            line: 1,
            message: "Agent loop detected without explicit max-steps or retry budget.",
            ruleId: "execution-missing-max-steps",
            severity: config.rules["execution-missing-max-steps"] || "warn",
            suggestion: "Add a max-steps limit or timeout to prevent runaway autonomy and infinite loops.",
    ...
    if (config.rules["architecture-atomic-transactions"] !== "off") {
        ...
        if (mutations > 1 && !content.includes("db.transaction")) {
            issues.push({
                file,
                line: 1,
                message: "Multiple database mutations detected without an atomic transaction.",
                ruleId: "architecture-atomic-transactions",
                severity: config.rules["architecture-atomic-transactions"] || "error",
                suggestion: "Wrap multiple database mutations in an atomic transaction (e.g., db.transaction()) to ensure data integrity.",
                category: "Execution Safety",
            });
        }
    }
    ...
    if (config.rules["execution-no-dry-run"] !== "off") {
        const content = lines.join("\n");
        if (/child_process\.exec|fs\.writeFileSync|db\.(insert|update|delete)/.test(content) &&
            ...
            issues.push({
                ...
                suggestion: "Implement a dry-run mode for dangerous tools to allow agents to preview side effects before committing.",
                category: "Execution Safety",
            });
        }
    }
